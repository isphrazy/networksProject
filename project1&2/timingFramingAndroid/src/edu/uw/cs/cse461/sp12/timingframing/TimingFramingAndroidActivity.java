...
public class TimingFramingAndroidActivity extends Activity implements Client.ClientListener {
    ...
    public void onToggleClicked(View v) {
    	if (((ToggleButton)v).isChecked()) {
			...
			if(portCheck.isChecked()){//use port
				if(portET.getText().toString().length() < 1){
					Toast.makeText(TimingFramingAndroidActivity.this, "port should not be empty", Toast.LENGTH_SHORT).show();
					return;
				}
				int portNumber = Integer.parseInt(portET.getText().toString());
				if(portNumber < 46101 || portNumber > 46109){
					Toast.makeText(TimingFramingAndroidActivity.this, "Please inter a number between 46101 to 46109", Toast.LENGTH_SHORT).show();
					return;
				}
			...
			}else{//use time interval specified
				if(interSymbolET.getText().toString().length() < 1){
					Toast.makeText(TimingFramingAndroidActivity.this, "interval should not be empty", Toast.LENGTH_SHORT).show();
					return;
				}
